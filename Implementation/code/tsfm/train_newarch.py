...
class Classify11(nn.Module):
    def __init__(self, padding_idx):
        ...
        self.ll1 = nn.Linear(94, edge_num)
        self.ll3 = nn.Linear(256, 94)
        self.ll2 = nn.Linear(272, 4)
    ...
    def forward(self, msp, edges):
        ...
        output = torch.cat((edges_part,msp_part),1)
        output = self.ll2(output).unsqueeze(0)
...
def train11(model, epoch, num):
    ...
    for batch, i in enumerate(range(0, len(num), batch_size)):
        ...
        accs += acc
            ...
            #print(preds_graph[0])
    print("epoch:", epoch)
    print("train mean_loss/acc: ", round(total_loss / len(num), 4), round(sum(accs) / len(accs), 4))
    ...
    with torch.no_grad():
        for i in range(0, len(num), batch_size):
            ...
            accs += acc
                ...
                #print(labels_graph[0])
                #print(preds_graph[0])
        print("valid mean_loss/acc: ", round(total_loss / len(num), 4), round(sum(accs) / len(accs), 4))
        valid_acc_list.append(round(sum(accs) / len(accs), 4))
        valid_loss_list.append(round(total_loss / len(num), 4))
        return total_loss / len(num), sum(accs) / len(accs)
...
def test11(model, epoch, num):
    ...
    with torch.no_grad():
        for i in range(0, len(num), batch_size):
            ...
            #print(labels_graph[0])
            #print(preds_graph[0])
        print("test mean_loss/acc: ", round(total_loss / len(num), 4), round(sum(accs) / len(accs), 4))
...
def train_transformer(epoch, num):
    val_loss_best = 999
    test_loss_bestval = 999
    test_acc_bestval = 999
    for i in range(1, 1 + epoch):
        cur_time = time.time()
        train11(model, i, num)
        #print("TrainTime",time.time() - cur_time)
        cur_time = time.time()
        val_loss, val_acc = evaluate11(model, i, range(1500, 1700))
        #print("EvalTime",time.time() - cur_time)
        cur_time = time.time()
        test_loss, test_acc = test11(model, i, range(1700, 1800))
        #print("TestTime",time.time() - cur_time)
        if val_loss_best > val_loss:
            val_loss_best = val_loss
            test_loss_bestval = test_loss
            test_acc_bestval = test_acc
        print("Best val loss | test loss | test acc :",val_loss_best,test_loss_bestval,test_acc_bestval)
        #print(model)
        norm_list = []
